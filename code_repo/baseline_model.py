from typing import List, Callable
import os, csv, random, time
import numpy as np
import matplotlib.pyplot as plt
import torch
import torch.nn as nn
from tqdm import trange, tqdm
from data_loaders import *

WINDOW_SIZE = 30
TRAIN_PATH = "../Data/train_stocks.csv"
VALID_PATH = "../Data/valid_data.npy"
VALID2_PATH = "../Data/valid2_data.npy"
TEST_PATH = "../Data/test_data.npy"
TEST2_PATH = "../Data/test2_data.npy"

train_loader = TrainLoader(TRAIN_PATH)
valid_loader = ValidTestLoader(VALID_PATH)
valid2_loader = ValidTestLoader(VALID2_PATH)
test_loader = ValidTestLoader(TEST_PATH)
test2_loader = ValidTestLoader(TEST2_PATH)

sample_loader = SampleLoader()

random.seed(time.time())

def batch_normalization(batch_x, batch_t):
    """
    Normalize batches as described below:
        
        Volume feature: 0-1 normalization
        Other features: relative difference from previous step, 
                        first step always == 1.0

    """
    
    # Normalize price-based features (min-max scaling)
    batch_x_min = np.min(batch_x[:,:,:-1], axis=1, keepdims=True)
    batch_t -= batch_x_min[:,0,3:4]
    batch_x[:,:,:-1] -= batch_x_min
    batch_x_max = np.max(batch_x[:,:,:-1], axis=1, keepdims=True)
    batch_x_max += (batch_x_max == 0)
    batch_t /= batch_x_max[:,0,3:4]
    batch_x[:,:,:-1] /= batch_x_max
    
    # Normalize volume
    #batch_t[:,-1] = batch_t[:,-1] / torch.max(batch_x[:,:,-1], axis=1)[0]
    #batch_x[:,:,-1] = batch_x[:,:,-1] / torch.unsqueeze(torch.max(batch_x[:,:,-1], axis=1)[0], dim=1)
    
    return batch_x, batch_t

class RNNModel(nn.Module):
    def __init__(self, hidden_size=1024, num_layers=2):
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        
        super().__init__()
        self.embedding = nn.Linear(5, hidden_size)
        self.rnn = nn.RNN(hidden_size, hidden_size, num_layers, batch_first=True)
        self.dense = nn.Linear(hidden_size, 1)
        
    def forward(self, x):
        x = self.embedding(x)
        x = self.rnn(x)[0]
        x = torch.max(x, dim=1)[0]
        x = self.dense(x)
        return x
    
class MLPModel(nn.Module):
    def __init__(self, hidden_sizes=[4096, 4096, 4096, 1]):
        super().__init__()
        
        hidden_sizes = [30*5] + hidden_sizes
        layers = [nn.Flatten()]
        for i in range(len(hidden_sizes) - 1):
            if i > 0:
                layers.append(nn.Sigmoid())
            layers.append(nn.Linear(hidden_sizes[i], hidden_sizes[i + 1]))
            
        self.layers = nn.Sequential(*layers)
        
    def forward(self, x):
        return self.layers(x)

def eval_loss(model, data_loader, loss_fn, normalize, batch_size=-1):
    """
    If num_batches == -1, returns the loss value as a Tensor with grad_fn
    
    Otherwise, returns the loss value as a float

    """
    x, t = data_loader.load_data()
    if normalize:
        x, t = batch_normalization(x, t)
    x, t = torch.Tensor(x), torch.Tensor(t)
    if torch.cuda.is_available():
        x = x.to('cuda')
        t = t.to('cuda')
        
    if batch_size != -1:
        xs = torch.split(x, batch_size)
        ts = torch.split(t, batch_size)
        loss = 0
        for i in range(len(xs)):
            y = model(xs[i])
            loss += loss_fn(y, ts[i]).item()
        loss /= len(xs)
    else:
        y = model(x)
        loss = loss_fn(y, t)
        
    return loss

def train_model(model, train_loader=train_loader, valid_loader=valid_loader, 
                valid2_loader=valid2_loader, seq_len=WINDOW_SIZE, 
                num_iters=20, lrate=1e-4, weight_decay=0, normalize=True):
    
    train_losses = []
    valid_losses = []
    valid2_losses = []
    
    loss_fn = (lambda y, t: 
               torch.sqrt(nn.MSELoss()(y, t))
               )
    optim = torch.optim.Adam(model.parameters(), lr=lrate, weight_decay=weight_decay)
    
    for i in range(num_iters):
        
        # Training step
        model.train()
        optim.zero_grad()
        train_loss = eval_loss(model, train_loader, loss_fn, normalize)
        train_loss.backward()
        optim.step()
            
        # Evaluation step
        model.eval()
        train_loss = eval_loss(model, train_loader, loss_fn, normalize).item()
        valid_loss = eval_loss(model, valid_loader, loss_fn, normalize, 1000)
        valid2_loss = eval_loss(model, valid2_loader, loss_fn, normalize, 1000)
        train_losses.append(train_loss)
        valid_losses.append(valid_loss)
        valid2_losses.append(valid2_loss)
        print(f"Iteration {i+1} | Train Loss {train_loss:.4f} | Valid Loss {valid_loss:.4f} | Valid2 Loss {valid2_loss:.4f}")
        
    return train_losses, valid_losses, valid2_losses

model = MLPModel().cuda()
train_losses, valid_losses, valid2_losses = train_model(model, lrate = 1e-4, num_iters=100)
plt.plot(train_losses, label="Train")
plt.plot(valid_losses, label="Valid")
plt.plot(valid2_losses, label="Valid2")
plt.legend()
plt.title("MLP Model")
plt.show()
    